...
int parseMetadata(parameter * & params, const int numparam, metadata & sim, cosmology & cosmo, icsettings & ic)
{
	...
	if (!parseParameter(params, numparam, "alpha_new", cosmo.alpha_new))
	{
		cosmo.alpha_new = P_alpha_new;
	}

	if (!parseParameter(params, numparam, "beta_new", cosmo.beta_new))
	{
		cosmo.beta_new = P_beta_new;
	}

	if (!parseParameter(params, numparam, "Omega_new", cosmo.Omega_new))
	{   
		cosmo.Omega_new = P_Omega_new;
	}

	if (!parseParameter(params, numparam, "phi_new", cosmo.phi_new))
	{
		cosmo.phi_new = P_phi_new;
	}
	...
	if (!parseParameter(params, numparam, "timestep_factor", sim.timestep_factor))
	{
		sim.timestep_factor = P_timestep_factor;
	}
	else if(sim.timestep_factor < P_timestep_factor)
	{
		sim.timestep_factor = P_timestep_factor;
	}
