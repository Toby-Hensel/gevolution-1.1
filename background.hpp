...
{	
	if (cosmo.full_cosmo)
	{
		return sqrt((2. * fourpiG / 3.) * (((cosmo.Omega_cdm + cosmo.Omega_b + bg_ncdm(a, cosmo)) / a) + (cosmo.Omega_Lambda * a * a) + (cosmo.Omega_rad / a / a))) * (1. + (cosmo.alpha_new / (cosmo.beta_new ? pow(a, cosmo.beta_new) : 1.) * sin(cosmo.Omega_new * ((1. / a) - 1.) + cosmo.phi_new)));
	}
	else 
...
inline double max_timestep(const double a, const double fourpiG, const double factor, const cosmology cosmo)
{
	if (cosmo.Omega_new)
		return 1. / fabs(cosmo.Omega_new) / factor * a / Hconf(a, fourpiG, cosmo);
